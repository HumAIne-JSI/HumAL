...
**Note on `instance_id`:** The ID is generated by a persistent DuckDB sequence (`al_instance_id_seq`). It is monotonic — never reused, even after the instance is deleted or the backend pod restarts. ID `0` is reserved for the ground-truth instance and is never returned by this endpoint.
