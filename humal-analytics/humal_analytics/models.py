"""
Data models for HumAL Analytics.
Mirrors the backend analytics_dm.py models for client-side use.
"""
...
class SessionComparison(BaseModel):
    ...
    # Rankings
    f1_ranking: list[int]
    efficiency_ranking: list[int]


# Legacy format models (for parsing external JSON files)
class LegacyDecision(BaseModel):
    """Decision format from external JSON logs."""
    t: float
    actor_type: str
    action: str
    payload: dict[str, Any] = Field(default_factory=dict)
...
class LegacySessionLog(BaseModel):
    """Session log format from external JSON files."""
    sim_id: str
    session_id: str
    pilot_tag: Optional[str] = None
    user_id: Optional[str] = None
    ...
    decisions: list[LegacyDecision] = Field(default_factory=list)


class LegacyLogsFile(BaseModel):
    """Root structure of external JSON log files."""
    logs: list[LegacySessionLog] = Field(default_factory=list)
